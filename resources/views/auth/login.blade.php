...
@section('content')
    ...
    @if (session('status'))
        <div
            class="mb-4 flex items-start gap-3 rounded-2xl bg-emerald-50/80 backdrop-blur-sm border border-emerald-100 px-4 py-3.5">
    ...
    <form method="POST" action="{{ route('login') }}" class="space-y-5">
        ...
        <div class="flex items-center justify-between">
            <label class="inline-flex items-center gap-2.5 cursor-pointer group">
                ...
                <span class="text-sm font-medium text-slate-600 group-hover:text-slate-900 transition-colors">Remember
                    me</span>
            </label>
        </div>
        ...
        <button type="submit"
            ...
            <div
                class="absolute inset-0 bg-gradient-to-r from-blue-500 to-violet-500 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
            </div>
        </button>
