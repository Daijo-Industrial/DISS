...
@section('content')
    ...
    <div class="print-container">
        ...
        <div class="no-print flex flex-wrap items-center justify-between gap-3 mb-6 pb-4 border-b border-slate-200">
            <div>
                <h1 class="text-xl font-bold text-slate-800">Forecast Material Prediction (Internal)</h1>
                <p class="text-sm text-slate-500">Preview and print the internal material forecast sheet.</p>
            </div>
            <div class="flex items-center gap-2">
                ...
                <a href="/foremind-detail/print/excel/{{ $vendorCode }}" class="inline-flex items-center gap-2 px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium rounded-lg shadow-sm transition">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <section class="invoice">
            <div class="flex items-center justify-between pb-3 mb-4 border-b border-slate-300">
                <div>
                    ...
                    <p class="text-xs text-slate-500 uppercase tracking-wider">Internal Material Forecast Report</p>
                </div>
                    ...
                    <table class="table table-bordered w-full text-left">
                        ...
                        <tbody>
                            @php
                                ...
                                $totalColumns = count($mon) + 6;
                            @endphp

                            @foreach ($materials as $key => $material)
                                <tr>
                                    @if ($material->material_code != $currentMaterialCode)
                                        <td class="table-bordered font-medium">{{ $material->material_code }}</td>
                                        <td class="table-bordered font-medium">{{ $material->material_name }}</td>
                                        @php
                                    ...
                                    <td class="table-bordered text-center">{{ $material->unit_of_measure }}</td>
                                    <td class="table-bordered text-right">{{ $material->quantity_material }}</td>
                                    ...
                                    @foreach ($qforecast[$loop->index] ?? [] as $index => $value)
                                        ...
                                        <td class="table-bordered text-right">
                                            <div class="text-slate-400 text-[11px]">{{ $value }}</div>
                                            <strong>{{ number_format($calculation, 2) }}</strong>
                                        </td>
                                    @endforeach

                                    <td class="table-bordered text-right font-bold">{{ number_format($total, 2) }}</td>
                                </tr>

                                @if ($loop->last)
                                    <tr class="bg-slate-50 font-bold">
                                        <td class="table-bordered" colspan="5"></td>
                                        <td class="table-bordered text-right">Monthly Total</td>
                                        @foreach ($monthlyTotals as $monthlyTotal)
                                            <td class="table-bordered text-right">
                                                <strong>{{ number_format($monthlyTotal, 2) }}</strong>
                                            </td>
                                        @endforeach
                                        <td class="table-bordered text-right font-bold">{{ number_format(array_sum($monthlyTotals), 2) }}</td>
                                    </tr>
                                @endif

                                @if (!$loop->last && isset($materials[$loop->index + 1]) && $material->material_code != $materials[$loop->index + 1]->material_code)
                                    <tr class="bg-slate-50 font-bold">
                                        <td class="table-bordered" colspan="5"></td>
                                        <td class="table-bordered text-right">Monthly Total</td>
                                        @foreach ($monthlyTotals as $monthlyTotal)
                                    ...
                                    <tr>
                                        <td class="table-bordered bg-slate-100" colspan="{{ $totalColumns }}" style="height: 6px; padding: 0 !important;"></td>
                                    </tr>
                                @endif
                            @endforeach
                        </tbody>
