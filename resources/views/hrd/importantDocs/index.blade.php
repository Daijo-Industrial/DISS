...
@section('content')
    <div class="px-4 py-4 space-y-4" x-data>
                ...
                document.addEventListener('alpine:init', () => {
                    Alpine.store('docLibrary', {
                        ...
                        async openDetail(id) {
                            ...
                            try {
                                const response = await fetch('{{ route('hrd.importantDocs.detail', '') }}/' +
                                    id, {
                                        headers: {
        ...
        <div class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden mt-6" x-data="{ filtersOpen: false }">
                ...
                <nav class="flex -mb-px gap-6" aria-label="Tabs">
                    ...
                    @foreach ($tabLinks as $t)
                        <a href="{{ route('hrd.importantDocs.index', ['tab' => $t['id'], 'threshold' => $threshold]) }}"
                            ...
                            <i
                                class="bx {{ $t['icon'] }} text-xl {{ $tab === $t['id'] ? $t['color'] ?? 'text-indigo-600' : '' }}"></i>
                            {{ $t['label'] }}
                            @if ($t['count'] > 0)
                                <span
                                    class="ml-1 rounded-lg px-2 py-0.5 text-[10px] font-black {{ $tab === $t['id'] ? 'bg-indigo-100 text-indigo-700' : 'bg-slate-100 text-slate-500 group-hover:bg-slate-200' }}">
                                    {{ $t['count'] }}
                                </span>
                            @endif
                        </a>
            ...
            <div
                class="px-6 py-4 border-b border-slate-100 bg-white flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                <div class="flex items-center gap-6">
                    {{-- Status Filter Pills (Context Aware) --}}
                    @if ($tab !== 'archived')
                        <div class="flex items-center gap-3">
                            <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Toggle
                                Status</span>
                            <div class="flex p-1 bg-slate-100 rounded-xl gap-1">
                                <button type="button" @click="$store.docLibrary.filterStatus('active')"
                                    ...
                                    :class="$store.docLibrary.activeStatus === 'active' ?
                                ...
                                <button type="button" @click="$store.docLibrary.filterStatus('expiring')"
                                    ...
                                    :class="$store.docLibrary.activeStatus === 'expiring' ?
                                        'bg-white text-amber-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'">
                                    Expiring
                                </button>
                                <button type="button" @click="$store.docLibrary.filterStatus('expired')"
                                    class="px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-tighter transition-all"
                                    :class="$store.docLibrary.activeStatus === 'expired' ? 'bg-white text-rose-600 shadow-sm' :
                ...
                <div class="flex items-center gap-3 relative">
                    ...
                    <div x-show="filtersOpen" @click.away="filtersOpen = false" x-cloak
                        ...
                        {{-- Category Block --}}
                        <div class="space-y-2">
                            <label class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Filter by
                                Category</label>
                        ...
                        <div class="space-y-2 pb-1">
                            <label class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]"
                                title="Docs turn Amber when expiry is closer than this limit">Warning Limit</label>
                            <form action="{{ route('hrd.importantDocs.index') }}" method="GET" class="relative group">
                                ...
                                <select name="threshold" onchange="this.form.submit()"
                                    ...
                                    @foreach ([1, 2, 3, 6] as $m)
                                        <option value="{{ $m }}" {{ $threshold == $m ? 'selected' : '' }}>
                                            {{ $m }} Months Notice</option>
                                    @endforeach
                                </select>
                            </form>
                        </div>
        ...
        <template x-teleport="body">
            <div x-show="$store.docLibrary.detailOpen" class="fixed inset-0 z-[150] overflow-hidden" style="display: none;"
                x-description="Slide-over panel, show/hide based on slide-over state.">
                <div class="absolute inset-0 overflow-hidden">
                    {{-- Background overlay --}}
                    <div x-show="$store.docLibrary.detailOpen" x-transition:enter="ease-in-out duration-500"
                            ...
                            <div class="flex h-full flex-col bg-slate-50 shadow-2xl ring-1 ring-slate-200">
                                ...
                                <div class="relative flex-1 overflow-y-auto">
                                    <template x-if="$store.docLibrary.detailLoading">
                                        <div
                                            class="absolute inset-0 flex items-center justify-center bg-slate-50/50 backdrop-blur-[2px] z-10">
                                            <div class="flex flex-col items-center gap-3">
                                                <div
                                                    class="h-10 w-10 border-4 border-indigo-600/20 border-t-indigo-600 rounded-full animate-spin">
                                                </div>
                                                <span
                                                    class="text-[10px] font-black uppercase tracking-widest text-slate-400">Loading
                                                    document...</span>
                                            </div>
                                        </div>
                                    </template>

                                    <div class="p-6 overflow-y-auto h-full" x-html="$store.docLibrary.sidebarHtml"
                                        x-show="!$store.docLibrary.detailLoading" x-transition.opacity>
                                    </div>
                                </div>
                            </div>
                        </div>
