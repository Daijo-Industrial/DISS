    ...
    <div class="mx-auto max-w-7xl px-3 py-6 sm:px-4 lg:px-0 space-y-6">
        ...
        <div class="glass-card overflow-hidden pt-8 pb-6 px-6 relative">
            ...
            <div class="relative z-10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                ...
                <div
                    class="relative z-10 flex flex-wrap items-center gap-2 bg-white/60 backdrop-blur-md p-1.5 rounded-2xl border border-slate-200/60 shadow-sm">
                    @php
                    ...
                    <div class="relative">
                        <select id="period-year"
                            class="form-select form-select-sm border-0 bg-transparent py-1.5 pl-3 pr-6 text-sm font-semibold text-slate-700 focus:ring-0 cursor-pointer w-auto shadow-none">
                            @if ($isElevated)
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4" id="status-summary-row">
            ...
            @foreach ($chips as $key => $chip)
                <div class="glass-card bg-white/80 p-4 border border-{{ $chip['color'] }}-100 group hover:-translate-y-1 hover:shadow-md transition-all duration-300 cursor-pointer stat-chip"
                    data-status="{{ $key }}">
                    <div class="flex items-start justify-between">
                        <div>
                            <p class="text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">
                                {{ $chip['label'] }}</p>
                            <h3 class="text-2xl font-black text-{{ $chip['color'] }}-600 tracking-tight"
                                data-chip="{{ $key }}">
                                {{ $summary[$key] ?? 0 }}
                            </h3>
                        </div>
        ...
        <div class="space-y-3" x-data="{ showLegend: false, showFormula: false }">
                        ...
                        <div class="space-y-1.5">
                            @foreach ($absenceLabels as $key => $info)
                                <div class="flex items-center gap-3">
                                    <span
                                        class="inline-flex items-center justify-center w-8 h-6 rounded font-bold text-xs
                                    ...
                                    <thead class="bg-slate-50">
                                        <tr>
                                            ...
                                            @foreach (['A', 'B', 'C', 'D'] as $g)
                                                <th class="px-3 py-2 text-center text-slate-500 font-semibold">
                                                    {{ $g }}</th>
                                            @endforeach
                                        </tr>
                                    </thead>
                                    <tbody class="divide-y divide-slate-100">
                                        @foreach ($oldMaps as $field => $scores)
                                            <tr class="hover:bg-slate-50">
                                                <td class="px-3 py-1.5 font-medium text-slate-700">
                                                    {{ $oldFieldLabels[$field] ?? $field }}</td>
                                                @foreach (['A', 'B', 'C', 'D'] as $g)
                                                    <td class="px-3 py-1.5 text-center text-indigo-600 font-bold">
                                                        {{ $scores[$g] ?? '—' }}</td>
                                                @endforeach
                                            </tr>
                                        @endforeach
                                        <tr class="bg-rose-50">
                                            <td class="px-3 py-1.5 font-semibold text-rose-700" colspan="5">
                                                Penalti: Alpha×{{ $penalties['alpha'] }} · Izin×{{ $penalties['izin'] }} ·
                                                Sakit×{{ $penalties['sakit'] }} · Telat×{{ $penalties['telat'] }}
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
            ...
            <div class="glass-card overflow-hidden shadow-sm border border-slate-200/60 relative mb-6"
                ...
                <div class="p-4 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    ...
                    {{-- Tabs --}}
                    <div class="flex items-center gap-1 p-1 bg-slate-100/80 rounded-xl w-full md:w-auto" role="tablist">
                        <template x-for="tab in tabs" :key="tab.id">
                            <button type="button" role="tab" :id="'tab-' + tab.id" :data-type="tab.id"
                                @click="switchTab(tab.id)" :aria-selected="activeTab === tab.id"
                                :class="activeTab === tab.id ?
                                    'bg-white text-slate-900 shadow-sm ring-1 ring-slate-900/5' :
                                    'text-slate-500 hover:text-indigo-600 hover:bg-white/50'"
                                class="flex-1 md:flex-none px-2 sm:px-4 py-2 text-[13px] sm:text-sm font-semibold rounded-lg transition-all duration-200 focus:outline-none text-center truncate">
                    ...
                    <div class="flex flex-wrap items-center gap-2" id="batch-action-bar">
                        ...
                        @if ($canApproveDept)
                            <form method="POST" action="{{ route('evaluation.approve-dept') }}" class="m-0"
                                id="approve-dept-form">
                                @csrf
                        ...
                        {{-- HRD: Final Approve --}}
                        @if ($canApproveFinal)
                            <form method="POST" action="{{ route('evaluation.approve-hrd') }}" class="m-0"
                                id="approve-hrd-form">
                                @csrf
                                <input type="hidden" name="month" value="{{ $month }}">
                                <input type="hidden" name="year" value="{{ $year }}">
                ...
                <div class="p-4">
                    ...
                    <div x-show="activeTab === 'regular'" x-cloak>
                        <div class="premium-datatable-wrapper overflow-x-auto custom-scrollbar pb-4 block w-full">
                            <table id="evaluation-regular-table" class="table table-hover table-striped w-full nowrap">
                            </table>
                        </div>
                    </div>
                    ...
                    <div x-show="activeTab === 'yayasan'" x-cloak>
                        <div class="premium-datatable-wrapper overflow-x-auto custom-scrollbar pb-4 block w-full">
                            <table id="evaluation-yayasan-table" class="table table-hover table-striped w-full nowrap">
                            </table>
                        </div>
                    </div>
                    ...
                    <div x-show="activeTab === 'magang'" x-cloak>
                        <div class="premium-datatable-wrapper overflow-x-auto custom-scrollbar pb-4 block w-full">
                            <table id="evaluation-magang-table" class="table table-hover table-striped w-full nowrap">
                            </table>
                        </div>
                    </div>
                </div>
            ...
            @role('super-admin')
                <div id="history-modal" class="fixed inset-0 z-[9999] hidden" aria-modal="true" role="dialog">
                    ...
                    <div class="absolute inset-y-0 right-0 w-full max-w-5xl bg-white shadow-2xl flex flex-col">
                            ...
                            <table class="w-full text-sm border-collapse" id="history-table">
                                <thead>
                                    <tr class="border-b border-slate-200 text-left">
                                        <th
                                            class="py-2 pr-3 font-semibold text-slate-500 text-xs uppercase tracking-wider whitespace-nowrap">
                                            Waktu</th>
                                        <th class="py-2 pr-3 font-semibold text-slate-500 text-xs uppercase tracking-wider">Aksi
                                            oleh</th>
                                        <th class="py-2 pr-3 font-semibold text-slate-500 text-xs uppercase tracking-wider">
                                            Deskripsi</th>
                                        <th class="py-2 pr-3 font-semibold text-slate-500 text-xs uppercase tracking-wider">Record
                                            ID</th>
                                        <th class="py-2 font-semibold text-slate-500 text-xs uppercase tracking-wider">Perubahan
                                        </th>
                                    </tr>
                                </thead>
                        ...
                        {{-- Pagination --}}
                        <div
                            class="px-6 py-3 border-t border-slate-100 bg-slate-50 flex items-center justify-between text-sm text-slate-500">
        ...
        (function() {
            ...
            ['approve-dept-form', 'approve-hrd-form'].forEach(id => {
                ...
                form.addEventListener('submit', function(e) {
                    ...
                    let alertText =
                        `Apakah Anda yakin ingin melakukan <strong>${actionText}</strong> untuk tab <strong>${typeLabel}</strong> bulan ini?`;
            ...
            (function() {
                ...
                function fetchHistory() {
                    ...
                    axios.get('{{ route('evaluation.history') }}', {
                        ...
                        if (!data.data.length) {
                            tbody.innerHTML =
                                '<tr><td colspan="5" class="py-8 text-center text-slate-400">Belum ada data history.</td></tr>';
                            infoEl.textContent = '';
                ...
                `).join('');
                        ...
                        infoEl.textContent =
                            `Halaman ${data.current_page} dari ${data.last_page} (${data.total} entri)`;
                        prevBtn.disabled = data.current_page <= 1;
                        nextBtn.disabled = data.current_page >= data.last_page;
                    }).catch(() => {
                        tbody.innerHTML =
                            '<tr><td colspan="5" class="py-8 text-center text-rose-400">Gagal memuat history.</td></tr>';
                    });
                }
            })();
